...
export function NavBar({ items, className }: NavBarProps) {
  ...
  return (
    <div
      className={cn(
        "pointer-events-none fixed left-1/2 z-50 -translate-x-1/2 bottom-6 sm:bottom-auto sm:top-6",
        className
      )}
    >
      <div className="pointer-events-auto flex items-center gap-3 rounded-full border border-border bg-background/5 px-1 py-1 shadow-lg backdrop-blur-lg">
        {items.map((item) => {
